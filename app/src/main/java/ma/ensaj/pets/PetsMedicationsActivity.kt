...
class PetsMedicationsActivity : AppCompatActivity() {
    ...
    private fun setupViews() {
        ...
        petsAdapter = PetsAdapter(
            ...
            onPetClick = { pet -> navigateToMedications(pet) } // Ajoutez cette ligne pour gérer le clic sur un animal
        )
            ...
            override fun onResponse(call: Call<List<Pet>>, response: Response<List<Pet>>) {
                if (response.isSuccessful) {
                    ...
                    if (petsList != null) {
                        ...
                        Toast.makeText(this@PetsMedicationsActivity, "No pets found", Toast.LENGTH_SHORT).show()
                    }
                } else {
                    if (response.code() == 401) {
                        sessionManager.clearSession()
                        startActivity(Intent(this@PetsMedicationsActivity, LoginActivity::class.java))
                        finish()
                    } else {
                        Toast.makeText(this@PetsMedicationsActivity, "Failed to load pets", Toast.LENGTH_SHORT).show()
                    }
                }
            }

            override fun onFailure(call: Call<List<Pet>>, t: Throwable) {
                Toast.makeText(this@PetsMedicationsActivity, "Network error: ${t.message}", Toast.LENGTH_SHORT).show()
            }
        })
    }
        ...
        pet.id?.let {
            petApi.deletePet(it).enqueue(object : Callback<Void> {
                override fun onResponse(call: Call<Void>, response: Response<Void>) {
                    if (response.isSuccessful) {
                        Toast.makeText(this@PetsMedicationsActivity, "Animal supprimé", Toast.LENGTH_SHORT).show()
                        loadPets(sessionManager.fetchUserId())
                    } else {
                        Toast.makeText(this@PetsMedicationsActivity, "Échec de la suppression", Toast.LENGTH_SHORT).show()
                    }
                }

                override fun onFailure(call: Call<Void>, t: Throwable) {
                    Toast.makeText(this@PetsMedicationsActivity, "Erreur réseau : ${t.message}", Toast.LENGTH_SHORT).show()
                }
            })
        }
    }
